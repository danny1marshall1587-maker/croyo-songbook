...
public class PerformanceGestures {
    ...
    public void scroll(boolean scrollDown) {
        if (mainActivityInterface.getMode().equals(c.getString(R.string.mode_presenter)) &&
            ...
            boolean sectionsViewMode = mainActivityInterface.getPreferences().getMyPreferenceBoolean("sectionsViewMode", false);
            if (sectionsViewMode && !mainActivityInterface.getSong().getFiletype().equals("PDF")) {
                jumpToSection(scrollDown);
            } else {
    ...
    private void jumpToSection(boolean scrollDown) {
        if (myZoomLayout == null) return;

        ArrayList<Integer> sectionHeights = mainActivityInterface.getSectionHeights();
        if (sectionHeights == null || sectionHeights.isEmpty()) return;

        float scale = myZoomLayout.getScaleFactor();
        int currentY = myZoomLayout.getScrollY();
        int nextY = 0;

        // Calculate cumulative heights to find the starts of each section
        ArrayList<Integer> sectionStarts = new ArrayList<>();
        int currentTotal = 0;
        for (int height : sectionHeights) {
            sectionStarts.add((int) (currentTotal * scale));
            currentTotal += height;
        }

        if (scrollDown) {
            // Find the first section that starts after the current position
            for (int start : sectionStarts) {
                if (start > currentY + 10) { // small buffer
                    nextY = start;
                    break;
                }
            }
            if (nextY == 0 && !sectionStarts.isEmpty()) {
                // If we are at the last section, we can't go further down
                nextY = currentY;
            }
        } else {
            // Find the first section that starts before the current position
            for (int i = sectionStarts.size() - 1; i >= 0; i--) {
        ...
        myZoomLayout.doScrollTo(0, nextY);
    }
