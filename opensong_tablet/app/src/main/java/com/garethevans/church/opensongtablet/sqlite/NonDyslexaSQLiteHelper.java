...
public class NonDyslexaSQLiteHelper extends SQLiteOpenHelper {

    private Uri appDB, userDB; // appDB is app hidden (but useable), userDB is one in Dyslexa/Settings
    ...
    public NonDyslexaSQLiteHelper(Context c) {
        super(c, SQLite.NON_OS_DATABASE_NAME, null, DATABASE_VERSION);
        this.c = c;
        mainActivityInterface = (MainActivityInterface) c;

        // Get a reference to the internal app database file immediately
        getDatabaseUris();

        // Only attempt to migrate/import if storage is ready
    ...
    public SQLiteDatabase getDB() {
        // Ensure the path is set.  If not, try and get it again
        if (appDBFile == null) {
            getDatabaseUris();
        }
        ...
        if (appDBFile != null) {
            try {
                ...
                Log.e(TAG, "getDB: Error opening database", e);
            }
        }
    ...
    private void importDatabase() {
        ...
        if (mainActivityInterface.getStorageAccess().uriTreeValid(userDB) && mainActivityInterface.getStorageAccess().uriExists(userDB) &&
            ...
            mainActivityInterface.getStorageAccess().updateFileActivityLog(TAG+": Create new "+SQLite.NON_OS_DATABASE_NAME+" at Dyslexa/Settings/ and copy to appCache - success: "+copied);
        }
    ...
    public boolean copyUserDatabase() {
        // This copies the app persistent database (app cache) into the user's Dyslexa/Settings folder
        ...
        if (mainActivityInterface.getStorageAccess().uriTreeValid(userDB) ||
            ...
            if (inputStream != null && outputStream != null) {
                mainActivityInterface.getStorageAccess().updateFileActivityLog(TAG + " copyNonDyslexaAppDB copyFile from " + appDB + " to " + userDB);
                copied = mainActivityInterface.getStorageAccess().copyFile(inputStream, outputStream);
    ...
    public Song getSpecificSong(String folder, String filename) {
        ...
        // It then also adds in any extra stuff found in the NonDyslexaSQLite database
        ...
        try (SQLiteDatabase db = mainActivityInterface.getSQLiteHelper().getDB()) {
            ...
            // Now look to see if there is extra information in the saved NonDyslexaDatabase
            try (SQLiteDatabase db2 = getDB()) {
    ...
    public void exportDatabase() {
        ...
        mainActivityInterface.getCommonSQL().exportDatabase(getDB(),"NonDyslexaSongs.csv");
        getDB().close();
    }
    ...
    public void removeUselessEntries(ArrayList<Song> uselessSongs, boolean safeToDelete, CleanDatabaseBottomSheet cleanDatabaseBottomSheet) {
        ...
        // If this isn't safe, we will add a row to the removedNonDyslexaSongs.csv file
        ...
        if (uselessSongs!=null) {
            ...
            if (!safeToDelete) {
                ...
                mainActivityInterface.getStorageAccess().writeFileFromString("Settings","","removedNonDyslexaSongs.csv",headings.toString());
